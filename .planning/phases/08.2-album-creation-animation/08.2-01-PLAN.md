...
2. Add thin border to imageContainer style for visual separation:
   - borderWidth: 0.5 (or 1 with StyleSheet.hairlineWidth)
   - borderColor: 'rgba(255, 255, 255, 0.15)' (subtle, not harsh)
3. Add `isHighlighted` prop (boolean, default false)
4. When isHighlighted transitions to true, run scale bounce animation:
...
5. Wrap the TouchableOpacity in an Animated.View with animatedStyle
6. Use useEffect to trigger animation when isHighlighted changes to true
...
<done>AlbumCard has subtle border for separation, animates with scale bounce when isHighlighted=true, AlbumBar exposes scrollToAlbum(albumId) method via ref</done>
</task>
