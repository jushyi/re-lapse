...
git commit -m "chore(19): add ESLint, Prettier, and pre-commit hooks

- Configure ESLint flat config with eslint-config-expo
- Add Prettier for consistent code formatting
- Set up Husky v9 + lint-staged pre-commit hooks
- Format entire codebase (48 source files)
...
- .prettierrc (new)
- .prettierignore (new)
- .husky/pre-commit (new/modified)
- package.json (updated scripts and lint-staged config)
- package-lock.json (new dependencies)
- All reformatted src/**/*.js files
- Any root config files that were reformatted

**Avoid:** Don't split into multiple commits - this is a single atomic change. Don't forget to stage .husky/ directory.
  </action>
  <verify>
- `git status` shows clean working tree after commit
- `git log -1 --stat` shows all expected files in commit
- `git diff HEAD~1..HEAD --stat` confirms scope of changes
  </verify>
